...
function iniciarCargaIncendio() {
  ...
  incendioState = { compartimentos: [], currentComp: null, fase: 0 };
...
function criarCompartimento() {
  const nome = document.getElementById('inc-comp-nome').value.trim();
  const area = parseFloat(document.getElementById('inc-comp-area').value);
...
function pedirActividade() {
  ...
  // Construir dropdown de actividades
  const optsFab = QUADRO2.filter(a => a.fab_qsi).map(a =>
    `<option value="fab_${QUADRO2.indexOf(a)}">[Fab] ${a.nome} (${a.fab_qsi} MJ/m²)</option>`
  ).join('');
  const optsArm = QUADRO2.filter(a => a.arm_qvi).map(a =>
    `<option value="arm_${QUADRO2.indexOf(a)}">[Arm] ${a.nome} (${a.arm_qvi} MJ/m³)</option>`
  ).join('');
  ...
  form.innerHTML = `
    ...
    <div class="form-row">
      <div class="form-field" style="flex:3">
        ...
        <select id="inc-act-sel" onchange="onActividadeChange()">
          <option value="">— Seleccione —</option>
          <optgroup label="Fabricação e Reparação">${optsFab}</optgroup>
          <optgroup label="Armazenamento">${optsArm}</optgroup>
        </select>
      </div>
    </div>
    <div class="form-row">
...
function onActividadeChange() {
  const sel = document.getElementById('inc-act-sel').value;
  const alturaWrap = document.getElementById('inc-act-altura-wrap');
  const info = document.getElementById('inc-act-info');
...
function adicionarActividade() {
  const sel = document.getElementById('inc-act-sel').value;
  const area = parseFloat(document.getElementById('inc-act-area').value);
  const ci_label = document.getElementById('inc-act-ci').value;
  ...
  if (isArm) {
    const altura = parseFloat(document.getElementById('inc-act-altura').value);
  ...
  // Limpar campos
  document.getElementById('inc-act-sel').value = '';
  document.getElementById('inc-act-area').value = '';
  document.getElementById('inc-act-altura').value = '';
  document.getElementById('inc-act-info').textContent = '';
  document.getElementById('inc-act-altura-wrap').style.display = 'none';
}

function renderActividadesList() {
  const list = document.getElementById('inc-act-list');
  if (!list) return;
  list.innerHTML = '';
